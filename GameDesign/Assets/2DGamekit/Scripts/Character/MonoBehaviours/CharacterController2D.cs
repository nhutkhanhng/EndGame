    ...
    public class CharacterController2D : MonoBehaviour
    {
        ...
        public void CheckCapsuleEndCollisions(bool bottom = true)
        {
            ...
            else
            {
                ...
                {
                    raycastDirection = Vector2.down;
                    Vector2 raycastStartBottomCentre = raycastStart + Vector2.down * (m_Box.size.y * 0.5f - m_Box.size.x * 0.5f);

                    m_RaycastPositions[0] = raycastStartBottomCentre + Vector2.left * m_Box.size.x * 0.5f;
                    m_RaycastPositions[1] = raycastStartBottomCentre;
                    m_RaycastPositions[2] = raycastStartBottomCentre + Vector2.right * m_Box.size.x * 0.5f;
                }
                else
                {
                    raycastDirection = Vector2.up;
                    Vector2 raycastStartTopCentre = raycastStart + Vector2.up * (m_Box.size.y * 0.5f - m_Box.size.x * 0.5f);

                    m_RaycastPositions[0] = raycastStartTopCentre + Vector2.left * m_Box.size.x * 0.5f;
                    m_RaycastPositions[1] = raycastStartTopCentre;
                    m_RaycastPositions[2] = raycastStartTopCentre + Vector2.right * m_Box.size.x * 0.5f;
                }
            }
            ...
            if (bottom)
            {
                ...
                else
                {
                    ...
                    if (m_Box != null)
                    {
                        ...
                        {
                            float capsuleBottomHeight = m_Rigidbody2D.position.y + m_Box.offset.y - m_Box.size.y * 0.5f;
